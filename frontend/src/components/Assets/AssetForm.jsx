...
const API_BASE = import.meta.env.VITE_API_URL || 'https://investfolio.onrender.com';

// Make sure to use default export
export default function AssetForm({ onClose, initialAsset = null }) {
  const { updateAsset, fetchAssets } = usePortfolio();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        // Use the context function for editing
        ...
        if (status) {
          // Use the fetchAssets function to update the UI immediately
          await fetchAssets();
